...
function normalizeRunningStatusData(raw: RunningStatusData): RunningStatusData {
  ...
  data.stations = data.stations.map((s, idx) => {
    ...
    const isNonStopping = rawStatus.toLowerCase().replace(/-/g, ' ') === 'non stopping';
    const status = isNonStopping
      ? 'Non-Stopping'
      : s.status || (s.delay_minutes ? `${s.delay_minutes} Min` : 'On Time');
  ...
  if (data.source && data.source.toUpperCase().includes('NTES')) {
