...
export async function startCaptchaSession(): Promise<CaptchaStartResponse> {
  const candidateEndpoints = [
    ...getCandidateApiUrls('/api/trains/captcha/start'),
    ...getCandidateApiUrls('/trains/captcha/start'),
    ...getCandidateApiUrls('/api/captcha/start'),
  ];
...
export async function refreshCaptcha(sessionId: string): Promise<CaptchaRefreshResponse> {
  const candidateEndpoints = [
    ...getCandidateApiUrls('/api/trains/captcha/refresh'),
    ...getCandidateApiUrls('/trains/captcha/refresh'),
    ...getCandidateApiUrls('/api/captcha/refresh'),
  ];
  const uniqueEndpoints = Array.from(new Set(candidateEndpoints));
  const baseUrl = getApiBaseUrl();
...
export async function searchTrains(payload: TrainSearchRequestBody): Promise<Train[]> {
  const candidateEndpoints = [
    ...getCandidateApiUrls('/api/trains/search'),
    ...getCandidateApiUrls('/trains/search'),
    ...getCandidateApiUrls('/api/search'),
  ];
  const uniqueEndpoints = Array.from(new Set(candidateEndpoints));
  const baseUrl = getApiBaseUrl();
...
export async function fetchTrainAvailability(
  ...
  const candidateEndpoints = [
    ...getCandidateApiUrls('/api/trains/availability'),
    ...getCandidateApiUrls('/trains/availability'),
    ...getCandidateApiUrls('/api/availability'),
  ];
  const uniqueEndpoints = Array.from(new Set(candidateEndpoints));
  const baseUrl = getApiBaseUrl();
