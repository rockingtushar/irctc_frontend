import { getApiBaseUrl } from './stations';
import {
...
export async function startCaptchaSession(): Promise<CaptchaStartResponse> {
  ...
  const candidateEndpoints = [
    `${baseUrl}/api/trains/captcha/start`,
    `${baseUrl}/trains/captcha/start`,
    `${baseUrl}/api/captcha/start`,
  ];
  ...
  if (lastError) {
    ...
    if (msg.includes('Failed to fetch') || msg.includes('NetworkError')) {
      throw new TrainApiError(
        `Backend server at ${baseUrl} is unreachable (Failed to fetch). Ensure your FastAPI server is running and port is forwarded as Public HTTPS.`,
        0
      );
    }
  ...
  throw new TrainApiError(`Could not reach CAPTCHA endpoint on ${baseUrl}. Check server status and URL configuration.`, 0);
}
...
export async function refreshCaptcha(sessionId: string): Promise<CaptchaRefreshResponse> {
  const baseUrl = getApiBaseUrl().replace(/\/$/, '');
  const candidateEndpoints = [
    `${baseUrl}/api/trains/captcha/refresh`,
    `${baseUrl}/trains/captcha/refresh`,
    `${baseUrl}/api/captcha/refresh`,
  ];
  ...
  if (lastError) {
    ...
    if (msg.includes('Failed to fetch') || msg.includes('NetworkError')) {
      throw new TrainApiError(
        `Backend server at ${baseUrl} is unreachable. Check your network or tunnel connection.`,
        0
      );
    }
...
export async function searchTrains(payload: TrainSearchRequestBody): Promise<Train[]> {
  const baseUrl = getApiBaseUrl().replace(/\/$/, '');
  const candidateEndpoints = [
    `${baseUrl}/api/trains/search`,
    `${baseUrl}/trains/search`,
    `${baseUrl}/api/search`,
  ];
  ...
  //   has_captcha: !!payload.captcha_answer,
  // });
  ...
  for (const url of candidateEndpoints) {
    try {
      ...
      if (response.status === 400) {
        ...
        // Incorrect captcha: SAME session kept
...
export async function fetchTrainAvailability(
  ...
  const baseUrl = getApiBaseUrl().replace(/\/$/, '');
  const candidateEndpoints = [
    `${baseUrl}/api/trains/availability`,
    `${baseUrl}/trains/availability`,
