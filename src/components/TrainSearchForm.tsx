...
import { preloadRoutesForTrains } from '../services/routePreloadService';
...
export const TrainSearchForm: React.FC<TrainSearchFormProps> = ({
  ...
  // Helper date generators synchronized with Indian Railways Time (IST)
  const getTodayString = () => {
    return getIndianRailwaysTodayString();
  };

  const getFutureDateString = (daysAhead: number) => {
    return getIndianRailwaysShiftedDate(daysAhead);
  };
  ...
  const executeSearch = async (
    ...
    // Live IRCTC search strictly requires solving CAPTCHA
  ...
  const handleSearchSuccess = (resultTrains: Train[]) => {
    ...
    // Preload complete route of every train immediately in the background
    preloadRoutesForTrains(resultTrains, journeyDate);
  };
  ...
  return (
    <div className="space-y-3 sm:space-y-6 w-full">
      <div className="bg-white/95 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-lg shadow-orange-500/5 border border-slate-200/90 p-4 sm:p-6 md:p-8 flex flex-col justify-between relative z-30" id="train-search-card">
        ...
        <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-2.5 sm:space-y-5" id="train-search-form">
            ...
            <div className="relative z-30 flex flex-col md:flex-row gap-1.5 sm:gap-2.5 md:gap-4 items-center">
              {/* FROM Station */}
              ...
              {/* Circular Swap Button - Centered between From & To stations */}
              <button
                type="button"
                ...
                className={`z-30 my-0.5 sm:my-0.5 md:my-0 md:-mx-3.5 md:mt-5 p-2 sm:p-2.5 md:p-3 bg-white border-2 border-slate-100 rounded-full shadow-md text-orange-500 hover:scale-110 active:scale-95 transition-all shrink-0 disabled:opacity-40 disabled:pointer-events-none cursor-pointer ${
                  isSwapping ? 'rotate-180' : 'rotate-0'
                }`}
              >
                <ArrowLeftRight className="w-4 h-4 sm:w-4.5 sm:h-4.5 md:w-5 md:h-5" />
              </button>

              {/* TO Station */}
              <div className="flex-1 w-full relative z-30 -mt-2 sm:-mt-1 md:mt-0">
                <StationAutocomplete
                  id={toInputId}
                  label="To Station"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4 pt-0.5 relative z-20">
              {/* Journey Date */}
              <div className="col-span-2 sm:col-span-1 relative z-20">
