...
export const CaptchaModal: React.FC<CaptchaModalProps> = ({
  ...
  const initCaptcha = useCallback(async () => {
    ...
    // Clear any stale saved train session from localStorage
    clearTrainSessionId();

    try {
      // Always start a brand-new, synchronized session
  ...
  const handleManualRefresh = async () => {
    ...
    try {
      // Start a guaranteed fresh session with fresh captcha
      clearTrainSessionId();
      const freshRes = await startCaptchaSession();
  ...
  const handleVerifyAndSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof InvalidCaptchaError) {
        // Start a fresh, clean session on invalid captcha so user is never stuck in stale session
        setCaptchaInput('');
        ...
        try {
          ...
          console.warn('[CaptchaModal] Fresh session after 400 failed:', freshErr);
        ...
        // HTTP 401: Discard old session, call startCaptchaSession for a brand-new session
        setSessionId(null);
        setCaptchaInput('');
